...
Both wire ends share one method set: seven client-to-server requests and four server-to-client notifications.
...
| client→server | `session/list` | `SessionListParams` → `SessionListResult` (newest-first descriptors) |
| client→server | `session/history` | `SessionHistoryParams` → `SessionHistoryResult` (raw log events) |
...
`session/list` and `session/history` read the deployment's `sessionQuery` service (an `sdk-minimal` deployment does not mount it and receives an explanatory error); `session/list` is newest-first, filters by exact `cwd`, caps with `limit`, and reports each entry's `live`/`persisted` flags together with its folded `session/title`. `session/history` returns the raw log in the same event vocabulary `session.event` carries — pass `limit` to fetch only the newest events, and read `truncated` to learn that older ones were dropped. `session/resume` makes a persisted session live again through `agents.resume`, so later `session/prompt` calls continue its history instead of creating a session; it never creates one (an unknown id rejects), it refuses a session whose recorded `cwd` differs from `initialize`'s (compared with `realpath`, disposing the resumed agent first) rather than run its tools somewhere the history does not describe, and it is an idempotent no-op for a session that is already live in the runtime. Resume does not replay history to the client: call `session/history` for that. `session/rename` appends an explicit user-owned `session/title` event through the deployment's `sessionTitle` service, so the accepted title is durable session state every front end reads rather than a client-local alias; it only accepts a session live in this runtime (a persisted session must be resumed first, because the service appends to the live instance) and rejects a title that normalizes to empty.
